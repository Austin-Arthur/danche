...
int32 closed_loop_pi_calc(int32 read_error)
{
    ...
    closed_loop.pout = closed_loop.error * (closed_loop.kp);

    //积分系数根据误差进行动态调节
    //closed_loop.iout += closed_loop.error * (closed_loop.ki + (float)myabs(closed_loop.error/1000)/38000);
    temp11 = closed_loop.iout;
    closed_loop.iout = temp11 + closed_loop.error * (closed_loop.ki );

    //积分限幅
    closed_loop.iout = (float)limit_ab(closed_loop.iout,0,closed_loop.out_max);
/*
