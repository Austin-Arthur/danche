...
uint8 control_hall[6]={5,4,6,2,3,1};        //霍尔时序矫正
...
inline void calc_speed(void)
{
    ...
    //2.commutation_time_sum是统计电机换相6次会进入多少次T12周期中断
        //2.1 T12周期中断周期中断的频率为20KHz   0.005ms
    ...
    speed = PWM_PIT_NUM/POLEPAIRS/commutation_time_sum;//速度约0.5-0.002ms出一次
    //speed = commutation_time_sum;
    motor_dir_out();
    if(REVERSE == motor_control.nowdir)//电机反转的时候需要对速度取反
    {
        speed = -speed;
    }
...
void scan_hall_status(void)
{
    ...
    if( ((hall_value_now != hall_value_last) && !commutation_delay) || !next_hall_value)        //开始换相
    {
        ...
        if((speed_filter.data_average > 200000) || (speed_filter.data_average < -200000))//900暂时关闭超前换向，应该有好处，但现在未发现好处，且容易换向失败
        {
//            速度大于一定的时候，霍尔延迟较大
