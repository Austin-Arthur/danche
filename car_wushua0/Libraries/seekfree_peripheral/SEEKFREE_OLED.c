...
//  @param      pointnum    小数位显示长度   最高6位
//------------------------------------------------------------------------------------------------------------------
void oled_printf_p6x8str_add_float(uint8 x,uint8 y,const int8 ch[],uint8 num_str,double dat,uint8 num_data,uint8 pointnum)
{
    oled_p6x8str(x,y,ch);
    oled_printf_float(x+num_str*6,y,dat,num_data,pointnum);
}
//------------------------------------------------------------------------------------------------------------------
//  @brief      OLED显示浮点数(去除整数部分无效的0)
//  @param      x           x轴坐标设置0-127
...
//  @param      ch[]        字符串
//  @param      num_str     字符串长度
//  @param      dat         需要显示的变量，数据类型int
//  @param      num_data         整数位显示长度   最高10位
//------------------------------------------------------------------------------------------------------------------
void oled_printf_p6x8str_add_int32(uint8 x,uint8 y,const int8 ch[],uint8 num_str,int32 dat,uint8 num_data)
{
    oled_p6x8str(x,y,ch);
    oled_printf_int32(x+num_str*6,y,dat,num_data);
}
...
/*
void LCD_MT9V032(uint8 pic[64][94])
{
  int i = 0, j = 0;
  uint8 temp;
  uart_putchar(UART_0,0xaa);
  systick_delay_us(STM0, 10);
  for(i=0;i<64;i++)//8*8=64行
  {
//      oled_set_pos(0,i/8);//设置当前显示位置
    for(j=0;j<88;j+=8)
    {
      temp=0;
      if(pic[i][j+7]) temp|=0x01;//0000 0001
      if(pic[i][j+6]) temp|=0x02;//0000 0010
      if(pic[i][j+5]) temp|=0x04;//0
      if(pic[i][j+4]) temp|=0x08;
      if(pic[i][j+3]) temp|=0x10;
      if(pic[i][j+2]) temp|=0x20;
      if(pic[i][j+1]) temp|=0x40;
      if(pic[i][j+0]) temp|=0x80;
//      if(j==47) temp = 0xaa;
