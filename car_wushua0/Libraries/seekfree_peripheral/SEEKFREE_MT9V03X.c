...
void V_isr() //5.1ms左右
{
    for(uint8 i=0;i<62;i++)//60       //比较二值化  2.4ms
    {
      for(uint8 j=0;j<94;j++)//94         软件二值化
      {
        if(mt9v03x_image[i][j]>CI.cnst)
        {
          image[i+1][j] = 1;//(1~62)
        }
        else
